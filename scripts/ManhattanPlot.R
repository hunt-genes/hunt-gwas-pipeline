...
option_list <- list(
  ...
  make_option("--maintitle", type="character", default="",
    help="Plot title"),
  make_option("--cutoff", type="numeric", default=6,
    help="Cutoff for considering SNPs as significant. Is an int, default 6 (10^cutoff)")
)
...
print(nrow(candidateRegions))
if (nrow(candidateRegions) == 0){
  print("Candidate regions file was empty!")
	candidateRegions <- data.frame(
		'CHROM'=character(0),
		'START'=numeric(0),
		'END'=numeric(0),
...
prethin1 <- which(gwas[[ycol]] >= -log10(0.05) & gwas[[ycol]] <= opt$cutoff)
...
keepTop <- which(gwas[[ycol]] > opt$cutoff)
